...
sim_circuit.V('clk',  "clk",  ground["name"] ,"DC 0V PULSE("+str(power["value"])+"V 0V 0n 1p 1p 1u 2u)")
sim_str = """
.tran {step} {max}
.control
set hcopydevtype = svg
run
plot v(clk) v(PCHG) v(WLEN) v(SAEN)
.endc
""".format(step=str(time_step)+time_unit, 
          max=str(max_time)+time_unit)#period
sim_circuit.raw_spice = sim_str

sim_circuit.X(0, 'ctrl',power["name"],ground["name"], "clk", "WREN", "PCHG","WLEN","SAEN")
f = open("ctrl_tb.spi", "w")
f.write(str(sim_circuit))
f.close()

#run simulation
subprocess.call(["ngspice","ctrl_tb.spi"])